...
class SchedulesData extends CodonData {
    ...
    public static function addSchedule($data) {
        ...
        $data['codeshare'] = strtoupper($data['codeshare']);
        $data['codenum'] = strtoupper($data['codenum']);
        if(isset($data['week1'])) {
    ...
    public static function getAllBids() {
        $sql = 'SELECT  p.*, s.*, c.*,
           ...
           ' . TABLE_PREFIX . 'codeshareflights c,
					 ' . TABLE_PREFIX . 'bids b,
					 ' . TABLE_PREFIX . 'aircraft a,
					 ' . TABLE_PREFIX . 'pilots p
				WHERE b.routeid = s.id AND b.routeid = c.id AND s.aircraft=a.id AND p.pilotid = b.pilotid
